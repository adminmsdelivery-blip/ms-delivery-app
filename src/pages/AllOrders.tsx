...
export default function AllOrders() {
  ...
  // Auto-calculate delivery_charges and estimated_profit when relevant fields change
  useEffect(() => {
    if (editingOrder && (editingOrder.total_amount_received !== undefined || editingOrder.item_charge !== undefined || editingOrder.outsource_charges !== undefined)) {
      const totalAmount = Number(editingOrder.total_amount_received) || Number(editingOrder.delivery_charges) || 0;
      const itemCharge = Number(editingOrder.item_charge) || 0;
      const outsourceCharges = Number(editingOrder.outsource_charges) || 0;
      
      const deliveryCharges = totalAmount - itemCharge;
      const estimatedProfit = deliveryCharges - outsourceCharges;
  ...
  const handleExportExcel = () => {
    const exportData = orders.map(order => ({
      ...
      'Delivery Location': order.delivery_location || order.drop_location,
      'Total Amount Received': order.total_amount_received || order.delivery_charges,
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      readOnly
                      className="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 text-gray-700 font-bold focus:ring-2 focus:ring-indigo-500 outline-none"
                      placeholder="0.00"
                      min="0"
                      step="0.01"
                    />
